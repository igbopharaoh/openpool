//! Canonical OPENPOOL-1 encodings, hashes, draw calculation, and public proof model.
...
pub fn entry_hash(data: &EntryData, previous_entry_hash: Hash32) -> Hash32 {
    ...
    digest.update(b"OPENPOOL-ENTRY-V1\0");
...
pub fn entries_root(raffle_id: RaffleId, entry_count: u64, entry_chain_head: Hash32) -> Hash32 {
    let mut digest = Sha256::new();
    digest.update(b"OPENPOOL-ENTRIES-V1\0");
...
pub fn draw_seed(input: DrawInput) -> Hash32 {
    let mut digest = Sha256::new();
    digest.update(b"OPENPOOL-DRAW-V1\0");
...
mod tests {
    ...
    fn ledger_and_draw_are_deterministic() {
        ...
        assert_eq!(
            ...
            "2d573580bd1670fe2a0dc43ac46d5a99f4f22352dbdda19f8fe215d56d677495"
        );
        assert_eq!(
            entries[1].entry_hash.to_hex(),
            "9bc84ff4275c5ebfb20ff4e893dc1506d7dcde645394293131618367b6638f64"
        );
        assert_eq!(
            summary.entries_root.to_hex(),
            "3bd5f9950e7637ab3668495374ab9376b416067bd3ff709bd4316836fc19f6ac"
        );
        assert_eq!(
            result.seed.to_hex(),
            "8bbced686c792c260b87024a2af6effb26a47110d27d09186205fc2e381e2da1"
        );
