...
impl Persistence {
    ...
    pub async fn public_proof_metadata(
        &self,
        raffle_id: RaffleId,
    ) -> Result<openpool_contract::PublicProofMetadata, PersistenceError> {
        let row = sqlx::query("SELECT proof_hash, storage_uri, storage_version_id, storage_etag, published_at FROM proofs WHERE raffle_id = $1 AND protocol_version = $2")
            ...
            .ok_or(PersistenceError::NotFound("proof"))?;
        Ok(openpool_contract::PublicProofMetadata {
    ...
    pub async fn public_result(
        &self,
        raffle_id: RaffleId,
    ) -> Result<openpool_contract::PublicResult, PersistenceError> {
        ...
        .fetch_one(&self.pool)
        .await?;
        Ok(openpool_contract::PublicResult {
            raffle_id: raffle_id.as_uuid(),
    ...
    pub async fn operator_jobs(
        &self,
    ) -> Result<Vec<openpool_contract::OperatorJob>, PersistenceError> {
        let rows = sqlx::query(
        ...
        Ok(rows
            .into_iter()
            .map(|row| openpool_contract::OperatorJob {
                id: row.get("id"),
                kind: row.get("kind"),
                status: row.get("status"),
    ...
    pub async fn public_pool(
        &self,
        raffle_id: RaffleId,
    ) -> Result<openpool_contract::PublicPool, PersistenceError> {
        let row = sqlx::query(
        ...
        .await?
        .ok_or(PersistenceError::NotFound("raffle"))?;
        Ok(openpool_contract::PublicPool {
            raffle_id: raffle_id.as_uuid(),
    ...
    pub async fn public_entries(
        ...
        cursor: u64,
        limit: u32,
    ) -> Result<openpool_contract::EntryPage, PersistenceError> {
        ...
        for row in rows.into_iter().take(limit as usize) {
            entries.push(openpool_contract::PublicEntry {
        ...
        Ok(openpool_contract::EntryPage {
            entries,
            next_cursor,
        })
    }
...
impl From<PublicRaffle> for openpool_contract::PublicRaffle {
...
impl From<PublicInvoice> for openpool_contract::PublicInvoice {
...
pub enum PersistenceError {
    ...
    #[error(transparent)]
    Domain(#[from] openpool_domain::DomainError),
    #[error(transparent)]
