...
use openpool_core::bitcoin::{BitcoinSource, Esplora};
use openpool_core::config::{AppConfig, Environment};
use openpool_core::domain::{EntryId, InvoiceId};
use openpool_core::jobs::{ClaimedJob, JobStore};
use openpool_core::payments::{
...
use openpool_core::proof_storage::{ProofStore, S3ProofStore, S3ProofStoreSettings};
...
async fn reconcile_payment(
    ...
    if job.kind == "raffle.draw" {
        ...
        let raffle_id = job
            ...
            .parse::<Uuid>()
            .map(openpool_core::domain::RaffleId::from)
            ...
            .map_err(|e| e.to_string())?
        {
            ...
            match provider
                .pay_invoice(PayoutRequest {
                    payout_id: openpool_core::domain::PayoutId::from(payout.id),
        ...
        .state
    {
        ...
        {
            Ok(_) => Ok(()),
            Err(openpool_core::persistence_sqlx::PersistenceError::RaffleNotOpen) => persistence
